...
def send_events_to_cribl_http(url, events, token, use_zip):
	"""
	Prepare newline-delimited JSON payload. If use_zip is True, gzip it and set
	the Content‑Encoding header; otherwise, send uncompressed payload.
	Returns the response object.
	"""
	payload = "\n".join(json.dumps(event) for event in events).encode('utf-8')
	if use_zip:
		payload_to_send = gzip.compress(payload)
		headers = {
	...
	response = requests.post(url, data=payload_to_send, headers=headers)
	print(f"[INFO] Cribl upload Status Code: {response.status_code}")
	print(f"[INFO] Cribl upload Response: {response.text}")
	return response


def upload_file(filename, events, cribl_http_endpoint, cribl_http_token, use_zip, max_attempts=5):
	"""
	...
	while attempt < max_attempts and not success:
		response = send_events_to_cribl_http(cribl_http_endpoint, events, cribl_http_token, use_zip)
...
def handle_chunk_results(all_events, new_events, max_events_per_file, cribl_http_endpoint, cribl_http_token, use_zip):
	"""
	...
	while len(all_events) >= max_events_per_file:
		...
		upload_file(filename, file_events, cribl_http_endpoint, cribl_http_token, use_zip)
	return all_events


def main(api_token, start_time_str, end_time_str, host_name, max_query_threads, max_events_per_file, max_minutes_per_chunk, cribl_http_endpoint, cribl_http_token, use_zip):
	"""
	...
	if max_query_threads == 1:
		...
		for c_start, c_end in time_chunks:
			...
			all_events = handle_chunk_results(all_events, chunk_events, max_events_per_file, cribl_http_endpoint, cribl_http_token, use_zip)
	...
	if all_events:
		...
		upload_file(filename, all_events, cribl_http_endpoint, cribl_http_token, use_zip)
...
def parse_args():
	...
	parser.add_argument("--zip", action="store_true", help="If present, use gzip compression for payload")
